  ...
  * with ages between the min and max, inclusive. In your result map, each 
  * key is an integer age, and the value for that key is a string with the 
  * names of all people at that age, separated by "and" if there is more 
  * than one person of that age. The order of names for a given age should
  * be in alphabetical order, such as "Bob and Carl" rather than "Carl and Bob".
  * (This is the order in which they naturally occur in the parameter map.) 
  * Include only ages between the min and max inclusive, where there is at 
  * least one person of that age in the parameter map. If the map passed is 
  * empty, or if there are no people in the map between the min/max ages, return an empty map.
  ...
  * You will need to construct a map to store your results, but you may not use any 
  * other structures (arrays, lists, etc.) as auxiliary storage. 
